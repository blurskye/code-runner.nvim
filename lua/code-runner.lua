...
function M.show_confirmation_popup(json_data, json_path, file_dir)
    ...
    M.confirmation_popup = Popup({
        enter = true,
        focusable = true,
        zindex = 50,  -- Ensure it's on top of other floating windows
        border = {
            style = "rounded",
            text = {
                top = " Confirm coderun.json ",
    ...
    local function set_content()
        ...
        table.insert(lines, "")
        table.insert(lines, "Do you want to use this configuration?")
        table.insert(lines, "Press 'y' to accept, 'n' or 'q' to reject and use default configuration.")
    ...
    local original_keymap = vim.api.nvim_buf_get_keymap(0, 'n')
    vim.api.nvim_buf_set_keymap(M.confirmation_popup.bufnr, 'n', '<Space>', '<NOP>', { noremap = true, silent = true })

    local function restore_keymap()
        for _, keymap in ipairs(original_keymap) do
            vim.api.nvim_buf_set_keymap(0, keymap.mode, keymap.lhs, keymap.rhs or '', {
    ...
    local function close_popup(accept)
        M.confirmation_popup:unmount()
        restore_keymap()
        M.confirmation_popup = nil
    ...
    -- Set up keybindings
    vim.api.nvim_buf_set_keymap(M.confirmation_popup.bufnr, 'n', 'y', 
        string.format([[<Cmd>lua require('code-runner').close_popup(true)<CR>]]), 
        { noremap = true, silent = true })
    
    vim.api.nvim_buf_set_keymap(M.confirmation_popup.bufnr, 'n', 'n', 
        string.format([[<Cmd>lua require('code-runner').close_popup(false)<CR>]]), 
        { noremap = true, silent = true })
    
    vim.api.nvim_buf_set_keymap(M.confirmation_popup.bufnr, 'n', 'q', 
        string.format([[<Cmd>lua require('code-runner').close_popup(false)<CR>]]), 
        { noremap = true, silent = true })

    -- Disable all other keybindings
    vim.api.nvim_buf_set_keymap(M.confirmation_popup.bufnr, 'n', '<Space>', '<NOP>', { noremap = true, silent = true })
    vim.api.nvim_buf_set_keymap(M.confirmation_popup.bufnr, 'n', '<CR>', '<NOP>', { noremap = true, silent = true })

    -- Close popup on BufLeave
    vim.api.nvim_create_autocmd("BufLeave", {
        buffer = M.confirmation_popup.bufnr,
        once = true,
        callback = function()
            close_popup(false)
        end,
...
return {
    setup = M.setup,
    bind_commands = M.bind_commands,
    unbind_commands = M.unbind_commands,
    load_json = M.load_json,
    send_interrupt = M.send_interrupt,
    complete_variables_in_commands = M.complete_variables_in_commands,
    close_popup = M.close_popup,  -- Add this line
}
