...
local function find_coderun_json(dir)
    ...
    elseif dir == "" or dir == "/" then
        ...
        return find_coderun_json(vim.fn.fnamemodify(dir, ":h"))
    end
end
...
function M.run_code()
    ...
    local coderun_json_path = find_coderun_json(file_dir)

    if coderun_json_path then
        local file = io.open(coderun_json_path, "r")
        local coderun = json.decode(file:read("*all"))
        file:close()

        for _, command in pairs(coderun) do
            cmd = command.command:gsub("$dir", file_dir)
            cmd = cmd:gsub("$fileNameWithoutExt", file_name_without_ext)
            cmd = cmd:gsub("$fileName", file_name)
            print("Running command: " .. cmd)
            vim.cmd("execute 'TermExec cmd=\"" .. cmd .. "\"'")
